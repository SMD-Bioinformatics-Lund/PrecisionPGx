...
class ArrangeHaplotype:
    ...
    def merge_data(self):
        ...
        self.detected_variants["multival_haplotype"] = self.detected_variants.ID.apply(
            lambda x: list(
                self.haplotype_definitions["HAPLOTYPE"][
        ...
        def _get_haplotypes(variant_subdf, current_haplotype, depth=2):
            ...
            if any(variant_subdf["CN"] == 0):
                ...
                variant_subdf["multival_haplotype"] = variant_subdf[
                    "multival_haplotype"
                ].apply(lambda x: remove_hap(x, current_haplotype))
            ...
            if len(variant_subdf) == 0 or not any(
                variant_subdf["multival_haplotype"].apply(lambda x: bool(x))
            ):
                wt_haplotype = "WT"
                return [
                    current_haplotype,
                    [_get_haplotypes(variant_subdf.copy(), wt_haplotype, depth - 1)],
                ]

            remaining_haplo = set(
                [
                    element
                    for halpolist in variant_subdf["multival_haplotype"]
            ...
            candidate_haplotypes = np.array(
                list(
                    set(
                        [
                            element
                            for halpolist in gene_subset["multival_haplotype"]
                            for element in halpolist
                        ]
                    )
                )
            ...
            order = list(
                reversed(
                    np.argsort(
                        [
                            sum(
                                gene_subset["multival_haplotype"].apply(
                                    lambda y: x in y
                                )
                            )
                            for x in candidate_haplotypes
                        ]
                    )
                )
            )
...
def main():
    ...
    ah = ArrangeHaplotype(
        variant_csv, haplotype_definitions, haplotype_activity, clinical_guidelines
    )
